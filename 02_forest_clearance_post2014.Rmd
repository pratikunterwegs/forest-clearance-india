...
# read in states
states <- st_read("Data/spatial/ne_10m_admin_1_states_provinces.shp") %>% 
  filter(admin == "India") %>% 
  mutate(name = stringr::str_to_title(name)) %>% 
  select(name) %>% 
  mutate(name = case_when(
    name == "Uttaranchal" ~ "Uttarakhand",
    T ~ name))
...
state_area <- appro_post_2014 %>% 
  ...
  mutate(state_name = case_when(
    state_name == "Dadar & Nagar Haveli" ~ "Dadra & Nagar Haveli",
...
# Create a plot
state_area_plot_post2014 <- ggplot(states) +
